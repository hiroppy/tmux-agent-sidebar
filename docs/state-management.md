...
11. Every pid-based teardown fails *closed*: only a process snapshot that positively lacks the agent counts as proof it exited. A failed `ps` or a pane with no pid is inconclusive, and clearing on inconclusive evidence permanently unregisters a live agent — `ActivityLog` re-asserts an absent `@pane_agent` during a turn, and Cursor adoption restores it on the next poll
