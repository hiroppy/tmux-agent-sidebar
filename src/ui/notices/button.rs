...
use crate::tmux::{CODEX_AGENT, CURSOR_AGENT};
...
/// next to `agent`. Codex and Cursor qualify — Claude's setup story is
...
pub(super) fn missing_hooks_has_copy_button(agent: &str) -> bool {
    matches!(agent, CODEX_AGENT | CURSOR_AGENT)
}
...
mod tests {
    ...
    #[test]
    fn missing_hooks_has_copy_button_for_hook_config_agents() {
        ...
        assert!(missing_hooks_has_copy_button("cursor"));
