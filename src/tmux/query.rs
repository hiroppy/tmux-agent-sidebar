...
fn parse_pane_fields_with_processes(
    ...
    // Codex / OpenCode / pi panes can leave stale tmux metadata behind
    // after the agent exits and the pane falls back to the user's shell.
    // None of the three expose a reliable "process exit" hook (Codex has
    // no such hook, OpenCode runs under Bun where `process.on("exit")`
    // does not fire our handlers, and pi's `session_shutdown` extension
    // event is not guaranteed to fire on a hard kill/pane close), so the
    // Rust polling side must own teardown: wipe pane options + activity
    // log the first poll after the agent is gone. Subsequent polls
    // short-circuit at the `AgentType::from_label` check above once
    // `@pane_agent` has been cleared. Claude is excluded because its
    // SessionEnd hook drives cleanup instead.
    if matches!(
        agent,
