...
Hold `Ctrl` and left-click a visible file row to open it in a tmux popup.

By default the popup runs `${EDITOR:-vim} {file}` from the repository root.
Set `EDITOR` to use a different editor such as `nvim`.
