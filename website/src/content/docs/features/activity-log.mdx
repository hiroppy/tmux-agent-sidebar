...
| Codex       | `Bash` only — Codex's `PostToolUse` fires only for Bash (its `tool_input` is typed `{ command: string }`), so `Read` / `Edit` / `Write` / `Grep` / `Glob` and other tool calls are **not** reported |
