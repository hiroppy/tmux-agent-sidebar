...
- **macOS** prefers [`terminal-notifier`](https://github.com/julienXX/terminal-notifier) when it is executable on `PATH`; install it with `brew install terminal-notifier`. The sidebar falls back to `osascript` only when `terminal-notifier` is unavailable.
- **Linux** uses `notify-send`.
- **Windows** is not supported yet.

If no supported notification command is available, the feature quietly disables itself.
